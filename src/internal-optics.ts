...
// The ray renderer handles actual nested transmission. Its materials must not
// inherit the alpha-compositing approximation used by WebGL's opaque capture.
export function configureTracedInternalOptics(mat: THREE.MeshPhysicalMaterial) {
  if (!mat.userData.opticalOrder) return;
  const density = mat.opacity;
  mat.attenuationColor.copy(mat.color);
  mat.color.lerp(new THREE.Color("#ffffff"), 0.75);
  mat.attenuationDistance = 0.25;
  mat.ior = 1.46;
  mat.transmission = 1 - density * 0.25;
  mat.thickness = 0.012;
  mat.opacity = 1;
  mat.transparent = false;
